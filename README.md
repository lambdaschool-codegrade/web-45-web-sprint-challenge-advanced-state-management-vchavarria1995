...
1. What problem does the context API help solve? Context API solves the problem of needing to use prop drilling.
2. In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application? Actions are objects that have a type field, they are sent to reducers whenever they are called. Reducers take actions and do what the action is. The store contains the state for the app. 
3. What does `redux-thunk` allow us to do? How does it change our `action-creators`? Thunk allows us to make API calls from our action creators by making them perform asynchronous API calls using middleware.
4. What is your favorite state management system you've learned and this sprint? Please explain why! So far it has been context API mainly cause of its ease of use.
